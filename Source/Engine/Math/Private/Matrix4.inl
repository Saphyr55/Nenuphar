...
Matrix4<R> Matrix4<R>::Rotate(Mat mat, const Real auto& theta, Vector3<R> vec)
{
	...
	(
        ...
        v.x * v.z * (1 - c) - v.y * s, v.z * v.y * (1 - c) + v.x * s, c + v.z * v.z * (1 - c), 0,
        0, 0, 0, 1
	);
}
...
Matrix4<R> Matrix4<R>::RotateY(const Float theta)
{
	return Mat
	(
		Vector4<R>(std::cos(theta), 0, -std::sin(theta), 0),
		Vector4<R>(0, 1, 0, 0),
...
template<Real R>
Matrix4<R>::Matrix4(const Vec& col_1, const Vec& col_2, const Vec& col_3, const Vec& col_4)
{
	value[0] = col_1;
	value[1] = col_2;
	value[2] = col_3;
